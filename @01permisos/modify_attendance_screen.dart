...
class _ModifyAttendanceScreenState extends State<ModifyAttendanceScreen> {
  ...
  Future<void> _checkAdminAccess() async {
    ...
    if (user == null || !RolePermissions.canModifyAttendance(user.role)) {
      _showAccessDenied();
      return;
    }
  ...
  Future<void> _loadPendingCount() async {
    try {
      final count = await _attendanceService.getPendingReviewCount();
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    if (_currentUser == null || !RolePermissions.canModifyAttendance(_currentUser!.role)) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Row(
          children: [
            const Text('Modificar Asistencias'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            // Filtro de pendientes de revisión
            Row(
              children: [
                Checkbox(
                ...
                const Text('Solo Pendientes de Revisión'),
                const SizedBox(width: 8),
                if (_showOnlyPending)
                  Container(
  ...
  Widget _buildEventCard(Map<String, dynamic> event) {
    ...
    final isPending = event['estado_revision'] == 'pendiente';
    ...
    return Card(
      ...
      child: Column(
        children: [
          ...
          // Botón de aprobar revisión (solo si está pendiente)
          if (isPending)
            Container(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () => _approveReview(event),
              ),
            ),
        ],
      ),
  ...
  Future<void> _approveReview(Map<String, dynamic> event) async {
    final userId = _authService.currentUserId;
    if (userId == null) {
    ...
    try {
      final success = await _attendanceService.approveAttendanceReview(
        eventId: event['id'],
        reviewerId: userId,
      );
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Revisión aprobada exitosamente'),
      ...
      _showError('Error aprobando revisión: $e');
    }
  }
