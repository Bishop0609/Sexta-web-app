        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              if (currentUserRole != null && RolePermissions.canCreateUsers(currentUserRole)) ...[
                                  ...
                                  DataCell(
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        if (currentUserRole != null && RolePermissions.canDeleteUsers(currentUserRole))
                                          IconButton(
                                            icon: const Icon(
                                              Icons.delete,
                                              size: 20,
...
class _UserFormDialogState extends State<_UserFormDialog> {
  ...
  final _victorNumberController = TextEditingController();
  final _registroCompaniaController = TextEditingController();

  String _selectedRank = 'Bombero';
  Gender _selectedGender = Gender.male;
  MaritalStatus _selectedMaritalStatus = MaritalStatus.single;
  UserRole _selectedRole = UserRole.bombero;
  ...
  void initState() {
    ...
    if (widget.user != null) {
      ...
      _enrollmentDate = widget.user!.enrollmentDate;
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final userMap = <String, dynamic>{
        ...
        'enrollment_date': _enrollmentDate?.toIso8601String().split('T')[0],
      };

      if (widget.user != null) {
        userMap['id'] = widget.user!.id;
        // Preservar campos de tesorería que no se editan en este dialog
        userMap['is_student'] = widget.user!.isStudent;
        userMap['payment_start_date'] = widget.user!.paymentStartDate?.toIso8601String();
        userMap['student_start_date'] = widget.user!.studentStartDate?.toIso8601String();
        userMap['student_end_date'] = widget.user!.studentEndDate?.toIso8601String();
        userMap['status'] = widget.user!.status.name;
      } else {
        userMap['id'] = '';
      }
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<UserRole>(
                        ...
                        items: const [
                          ...
                          DropdownMenuItem(value: UserRole.oficial1, child: Text('Oficial 1 - Capitán y Tte 1')),
                          DropdownMenuItem(value: UserRole.oficial2, child: Text('Oficial 2 - Gestión Actividades')),
                          DropdownMenuItem(value: UserRole.oficial3, child: Text('Oficial 3 - Ayudante')),
                          DropdownMenuItem(value: UserRole.oficial4, child: Text('Oficial 4 - Teniente a cargo de EPP')),
                          DropdownMenuItem(value: UserRole.oficial5, child: Text('Oficial 5 - Solo administración de EPP')),
                          DropdownMenuItem(value: UserRole.oficial6, child: Text('Oficial 6 - Tesorero')),
                          DropdownMenuItem(value: UserRole.bombero, child: Text('Bombero')),
                        ],
                        onChanged: (value) {
                          if (value != null) {
                      ...
                      InkWell(
                        onTap: () async {
                          final date = await showDatePicker(
                            ...
                            lastDate: DateTime.now(),
                            locale: const Locale('es', 'CL'),
                            helpText: 'Seleccionar Fecha de Ingreso (Juramento)',
                          );
                          if (date != null) {
                        ...
                        child: InputDecorator(
                          decoration: const InputDecoration(
                            labelText: 'Fecha de Ingreso (Juramento)',
                            helperText: 'Corresponde a la fecha de juramento como bombero.',
                            border: OutlineInputBorder(),
                            contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                          ),
                          child: Row(
                            ...
                            children: [
                              Text(
                                _enrollmentDate != null
                                    ? '${_enrollmentDate!.day.toString().padLeft(2, '0')}/${_enrollmentDate!.month.toString().padLeft(2, '0')}/${_enrollmentDate!.year}'
                                    : 'No especificada',
                              ),
                              const Icon(Icons.calendar_today, size: 20),
                            ],
